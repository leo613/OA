...
<script>
    ...
    function getSelectionsIds(){
    	...
    	var sels = itemList.datagrid("getSelections"); //获取数据网格数据
    	var ids = []; //创建数组
    	for(var i in sels){
    		ids.push(sels[i].id); //遍历循环得到 id
    	}
    	ids = ids.join(",");  //以“，”分开
    	return ids;
    }
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	if(ids.indexOf(',') > 0){ //判断后面有没有","如果有代表不止一个数据
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			//回显数据
        			var data = $("#itemList").datagrid("getSelections")[0]; //拿到所选择的信息
        			data.priceView = TAOTAO.formatPrice(data.price);  //格式化价格
        			$("#itemeEditForm").form("load",data); //将格式化后的价格回显到页面
                    ...
                    // $.ajax({
                    //     type: "GET",
                    //     url: "/rest/item/param/item/query/" + node.id,
                    //     statusCode : {
                    //         200 : function(data){
                    //             $("#"+formId+" .params").show();
